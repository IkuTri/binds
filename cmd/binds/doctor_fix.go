...
func previewFixes(result doctorResult) {
	...
	fmt.Println("Run 'binds doctor --fix' to apply these fixes")
}
	...
	for _, check := range fixes {
		...
		case "Child-Parent Dependencies":
			...
			if !doctorFixChildParent {
				fmt.Printf("  ⚠ Child→parent deps require explicit opt-in: binds doctor --fix --fix-child-parent\n")
				continue
			}
			err = fix.ChildParentDependencies(path, doctorVerbose)
		case "Duplicate Issues":
			// No auto-fix: duplicates require user review
			fmt.Printf("  ⚠ Run 'binds duplicates' to review and merge duplicates\n")
			continue
		case "Test Pollution":
			// No auto-fix: test cleanup requires user review
			fmt.Printf("  ⚠ Run 'binds doctor --check=pollution' to review and clean test issues\n")
			continue
		case "Git Conflicts":
		...
		case "Compaction Candidates":
			...
			fmt.Printf("  ⚠ Run 'binds compact --analyze' to review candidates\n")
			continue
		case "Large Database":
			// No auto-fix: pruning deletes data, must be user-controlled
			fmt.Printf("  ⚠ Run 'binds cleanup --older-than 90' to prune old closed issues\n")
			continue
