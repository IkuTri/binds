...
// This allows respecting the sync.remote binds config.
// Uses RepoContext to ensure git commands run in the correct repository.
func gitPull(ctx context.Context, configuredRemote string) error {
	...
	// 1. If configuredRemote (from sync.remote binds config) is set, use that
...
func gitPush(ctx context.Context, configuredRemote string) error {
	...
	if configuredRemote != "" {
		...
		cmd := rc.GitCmd(ctx, "push", configuredRemote, branch) //nolint:gosec // G204: configuredRemote from binds config
		output, err := cmd.CombinedOutput()
		if err != nil {
