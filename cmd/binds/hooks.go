...
func FormatHookWarnings(statuses []HookStatus) string {
	...
	if missingCount > 0 {
		...
		warnings = append(warnings, "   Run: binds hooks install")
	}
...
func installHooksWithOptions(embeddedHooks map[string]string, force bool, shared bool, chain bool, beadsHooks bool) error {
	...
	for hookName, hookContent := range embeddedHooks {
		...
		if _, err := os.Stat(hookPath); err == nil {
			if chain {
				// Chain mode - rename to .old so binds hooks run can call it
				...
				if verr != nil || !versionInfo.IsBdHook {
					...
					// This prevents destroying user's original hook if binds hooks install --chain
...
// =============================================================================
// Hook Implementation Functions (called by thin shims via 'binds hooks run')
// =============================================================================
...
func runChainedHook(hookName string, args []string) int {
	...
	// This can happen if user runs `binds hooks install --chain` multiple times,
...
func runPostCheckoutHook(args []string) int {
	...
	if isGitWorktree() {
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "╔══════════════════════════════════════════════════════════════════════════╗")
		fmt.Fprintln(os.Stderr, "║ Welcome to binds in git worktree!                                        ║")
		fmt.Fprintln(os.Stderr, "╠══════════════════════════════════════════════════════════════════════════╣")
		...
		fmt.Fprintln(os.Stderr, "║   export BINDS_NO_DAEMON=1                                               ║")
		fmt.Fprintln(os.Stderr, "║                                                                          ║")
		fmt.Fprintln(os.Stderr, "║ For more information:                                                    ║")
		fmt.Fprintln(os.Stderr, "║   - Run: binds doctor                                                       ║")
