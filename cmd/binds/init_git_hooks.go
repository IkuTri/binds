...
func hooksInstalled() bool {
	...
	// Verify they're binds hooks by checking for signature comment
	// #nosec G304 - controlled path from git directory
	preCommitContent, err := os.ReadFile(preCommit)
	if err != nil || !strings.Contains(string(preCommitContent), "binds (beads) pre-commit hook") {
		return false
	}

	// #nosec G304 - controlled path from git directory
	postMergeContent, err := os.ReadFile(postMerge)
	if err != nil || !strings.Contains(string(postMergeContent), "binds (beads) post-merge hook") {
		return false
	}
...
func detectExistingHooks() []hookInfo {
	...
	for i := range hooks {
		...
		if err == nil {
			...
			hooks[i].isBdHook = strings.Contains(hooks[i].content, "binds (beads)")
...
func installGitHooks() error {
	...
	// Check if any non-binds hooks exist
	...
	if chainHooks {
		fmt.Printf("%s Chained binds hooks with existing hooks\n", ui.RenderPass("✓"))
	}
...
func buildPreCommitHook(chainHooks bool, existingHooks []hookInfo) string {
	if chainHooks {
		...
		return `#!/bin/sh
#
# binds (beads) pre-commit hook (chained)
#
# This hook chains binds functionality with your existing pre-commit hook.
...
if ! bd sync --flush-only >/dev/null 2>&1; then
    ...
    echo "Run 'binds sync --flush-only' manually to diagnose" >&2
    exit 1
fi
...
func buildPostMergeHook(chainHooks bool, existingHooks []hookInfo) string {
	if chainHooks {
		...
		return `#!/bin/sh
#
# binds (beads) post-merge hook (chained)
#
# This hook chains binds functionality with your existing post-merge hook.
...
if ! bd import -i "$BEADS_DIR/issues.jsonl" >/dev/null 2>&1; then
    ...
    echo "Run 'binds import -i $BEADS_DIR/issues.jsonl' manually to see the error" >&2
fi
...
// mergeDriverInstalled checks if binds merge driver is configured correctly
...
func installJJHooks() error {
	...
	if chainHooks {
		fmt.Printf("%s Chained binds hooks with existing hooks (jj mode)\n", ui.RenderPass("✓"))
	}
...
func buildJJPreCommitHook(chainHooks bool, existingHooks []hookInfo) string {
	if chainHooks {
		...
		return `#!/bin/sh
#
# binds (beads) pre-commit hook (chained, jujutsu mode)
#
# This hook chains binds functionality with your existing pre-commit hook.
...
func printJJAliasInstructions() {
	...
	fmt.Printf("Jujutsu doesn't support hooks yet. To auto-export binds on push,\n")
...
func installMergeDriver() error {
	...
	cmd = exec.Command("git", "config", "merge.beads.name", "binds JSONL merge driver")
	if output, err := cmd.CombinedOutput(); err != nil {
	...
	if !hasBeadsMerge {
		...
		beadsMergeAttr := "\n# Use binds merge for beads JSONL files\n.beads/issues.jsonl merge=beads\n"
