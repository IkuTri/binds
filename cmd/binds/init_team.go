...
func runTeamWizard(ctx context.Context, store storage.Storage) error {
	...
	fmt.Println("This wizard will configure binds for team collaboration.")
	fmt.Println()
	...
	if protectedMain {
		...
		fmt.Println("\n  Binds will commit issue updates to a separate branch.")
	...
	if autoSync {
		...
		fmt.Printf("%s Auto-sync disabled (manual sync with 'binds sync')\n", ui.RenderWarn("⚠"))
	}
	...
	fmt.Println("  • Use 'binds list' to see all team's issues")
	...
	if autoSync {
		...
		fmt.Println("  • Run 'binds sync' manually to sync changes")
	}

	fmt.Println()
	fmt.Printf("Try it: %s\n", ui.RenderAccent("binds create \"Team planning issue\" -p 2"))
	fmt.Println()
