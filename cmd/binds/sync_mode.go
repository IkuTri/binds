...
// GetSyncMode returns the configured sync mode, checking config.yaml first (where binds config set writes),
...
func GetSyncMode(ctx context.Context, s storage.Storage) string {
	// First check config.yaml (where binds config set writes for sync.* keys)
