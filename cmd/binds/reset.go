...
var resetCmd = &cobra.Command{
	Use:   "reset",
	Short: "Remove all binds data and configuration",
	Long: `Reset binds to an uninitialized state, removing all local data.
...
func runReset(cmd *cobra.Command, args []string) {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		if jsonOutput {
			outputJSON(map[string]interface{}{
				"message": "binds not initialized",
				"reset":   false,
			})
		} else {
			fmt.Println("Binds is not initialized in this repository.")
...
func collectResetItems(gitCommonDir, beadsDir string) []resetItem {
	...
	if hasGitattributesEntry() {
		items = append(items, resetItem{
			Type:        "gitattributes",
			Path:        ".gitattributes",
			Description: "Remove binds entry from .gitattributes",
		})
	}
...
func showResetPreview(items []resetItem) {
	...
	fmt.Println()
	fmt.Printf("To proceed, run: %s\n", ui.RenderWarn("binds reset --force"))
}
...
func removeGitattributesEntry() error {
	...
	for _, line := range lines {
		...
		if strings.Contains(line, "Use binds merge for beads JSONL files") {
			skipNextEmpty = true
			continue
		}
