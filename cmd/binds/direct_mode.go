...
// Daemon has been removed; this is now equivalent to ensureStoreActive.
func ensureDirectMode(_ string) error {
	return ensureStoreActive()
}

// fallbackToDirectMode ensures a local store is ready.
// Daemon has been removed; reason parameter is retained for call-site compatibility.
func fallbackToDirectMode(_ string) error {
	return ensureStoreActive()
}

// disableDaemonForFallback is a no-op; daemon has been removed.
// Retained for call-site compatibility.
func disableDaemonForFallback(_ string) {
	// No-op: daemon removed. Direct mode is always active.
}
...
func ensureStoreActive() error {
	...
	if beadsDir == "" {
		return fmt.Errorf("no binds database found.\n" +
			"Hint: run 'binds init' to create a database in the current directory,\n" +
			"      or use 'binds --no-db' for JSONL-only mode")
	}
	...
	if _, err := os.Stat(jsonlPath); err == nil {
		...
		if isNoDbModeConfigured(beadsDir) {
			return fmt.Errorf("this project uses JSONL-only mode (no SQLite database).\n" +
				"Hint: use 'binds --no-db <command>' or set 'no-db: true' in config.yaml")
		}
	}
	...
	if err != nil {
		...
		if _, statErr := os.Stat(jsonlPath); statErr == nil {
			return fmt.Errorf("found JSONL file but no database: %s\n"+
				"Hint: run 'binds init' to create the database and import issues,\n"+
				"      or use 'binds --no-db' for JSONL-only mode", jsonlPath)
		}
