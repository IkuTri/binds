...
Subcommands:
  ...
  tombstones  Convert deletions.jsonl to inline tombstones`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		FatalErrorWithHint("no .beads directory found", "run 'binds init' to initialize bd")
		}
		}
	...
	// Detect all database files (SQLite backend only)
		...
		if len(databases) == 0 {
			if jsonOutput {
				...
				fmt.Fprintf(os.Stderr, "Run 'binds init' to create a new database.\n")
			}
			return
		}
...
func handleDoltMetadataUpdate(cfg *configfile.Config, beadsDir string, dryRun bool) {
	...
	if err != nil || !info.IsDir() {
		if jsonOutput {
			...
			fmt.Fprintf(os.Stderr, "Run 'binds init' to create a new database, or 'binds migrate --to-dolt' to migrate from SQLite.\n")
		}
		return
	}
...
func handleUpdateRepoID(dryRun bool, autoYes bool) {
	...
	if beadsDir == "" {
		if jsonOutput {
			outputJSON(map[string]interface{}{
				"error":   "no_database",
				"message": "No beads database found. Run 'binds init' first.",
			})
		} else {
...
func handleInspect() {
	...
	if !dbExists {
		result := map[string]interface{}{
			...
			"warnings":            []string{"Database does not exist - run 'binds init' first"},
		...
		if jsonOutput {
			...
			fmt.Println("\n⚠ Database does not exist - run 'binds init' first")
		}
		return
	}
	...
	if _, err := os.Stat(targetPath); os.IsNotExist(err) {
		if jsonOutput {
			outputJSON(map[string]interface{}{
				...
				"message": "Database not found. Run 'binds init' first.",
			})
		} else {
