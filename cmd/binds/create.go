...
var createCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if strings.HasPrefix(strings.ToLower(title), "test") && !silent && !debug.IsQuiet() {
			...
			fmt.Fprintf(os.Stderr, "  For testing, consider using: BINDS_DB=/tmp/test.db ./binds create \"Test issue\"\n")
		}
		...
		if explicitID != "" {
			...
			// Get database prefix and allowed prefixes from config (GH#1145: fallback to config.yaml)
		...
		// so the issue appears in binds list when hydration is enabled (bd-fix-routing)
		if repoPath != "." {
