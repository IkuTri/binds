...
import (
	...
	"github.com/IkuTri/binds/cmd/binds/setup"
	"github.com/IkuTri/binds/internal/recipes"
)
...
Use 'binds setup <recipe> --check' to verify installation status.
Use 'binds setup <recipe> --remove' to uninstall.`,
...
func listRecipes() {
	...
	fmt.Println()
	fmt.Println("Use 'binds setup <recipe>' to install.")
	fmt.Println("Use 'binds setup --add <name> <path>' to add a custom recipe.")
}
...
func runRecipe(name string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		fmt.Fprintln(os.Stderr, "Use 'binds setup --list' to see available recipes.")
		os.Exit(1)
	}
