...
// or if hooks were manually installed via 'binds setup claude'.
func isClaudeSetupComplete() bool {
	...
	// Check for manual hooks installation via 'binds setup claude'
	...
	for _, event := range []string{"SessionStart", "PreCompact"} {
		...
		for _, hook := range eventHooks {
			...
			for _, cmd := range commands {
				...
				if cmdStr == "binds prime" || cmdStr == "binds prime --stealth" {
					return true
				}
			}
		}
...
func initDefaultTips() {
	...
	InjectTip(
		"claude_setup",
		"Install the beads plugin for automatic workflow context, or run 'binds setup claude' for CLI-only mode",
	...
	InjectTip(
		"sync_conflict",
		"Run 'binds sync' to resolve sync conflict",
