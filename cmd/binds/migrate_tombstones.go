  ...
  bd migrate-tombstones --verbose # Show detailed progress`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if jsonOutput {
			...
			fmt.Println("\nNext steps:")
			fmt.Println("  1. Run 'binds sync' to propagate tombstones to remote")
...
func init() {
	...
	migrateTombstonesAliasCmd.Deprecated = "use 'binds migrate tombstones' instead (will be removed in v1.0.0)"
