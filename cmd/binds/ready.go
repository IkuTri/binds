...
behavior of 'binds list --ready' and shows work that is truly available to claim.
...
This is useful for agents executing molecules to see which steps can run next.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		filter := types.WorkFilter{
			Status:          "open", // Only show open issues, not in_progress (matches binds list --ready)
			Type:            issueType,
			Limit:           limit,
...
// ParallelInfo holds parallel execution analysis for a molecule step.
type ParallelInfo struct {
	IsReady       bool     `json:"is_ready"`
	CanParallel   []string `json:"can_parallel,omitempty"`
	ParallelGroup string   `json:"parallel_group,omitempty"`
}

// moleculeParallelAnalysis holds the result of parallel analysis for a molecule.
type moleculeParallelAnalysis struct {
	Steps          map[string]*ParallelInfo
	TotalSteps     int
	ParallelGroups map[string][]string
}

// analyzeMoleculeParallel returns a stub analysis (molecule/gate features removed).
func analyzeMoleculeParallel(subgraph *TemplateSubgraph) *moleculeParallelAnalysis {
	analysis := &moleculeParallelAnalysis{
		Steps:          make(map[string]*ParallelInfo),
		ParallelGroups: make(map[string][]string),
		TotalSteps:     len(subgraph.Issues),
	}
	for _, issue := range subgraph.Issues {
		analysis.Steps[issue.ID] = &ParallelInfo{IsReady: issue.Status == "open"}
	}
	return analysis
}
