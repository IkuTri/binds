...
// showDetectPollutionDeprecationHint shows a hint about binds doctor consolidation
func showDetectPollutionDeprecationHint() {
	fmt.Fprintln(os.Stderr, ui.RenderMuted("💡 Tip: Use 'binds doctor --check=pollution' instead (this command is deprecated)"))
}

var detectPollutionCmd = &cobra.Command{
	Use:        "detect-pollution",
	GroupID:    "maint",
	Hidden:     true, // deprecated, use 'binds doctor --check=pollution' instead
	Deprecated: "use 'binds doctor --check=pollution' instead (will be removed in v1.0.0)",
...
EXAMPLES:
  binds detect-pollution                 # Show potential test issues
  binds detect-pollution --clean         # Delete test issues (with confirmation)
  binds detect-pollution --clean --yes   # Delete without confirmation
  binds detect-pollution --json          # Output in JSON format
...
NOTE: Review detected issues carefully before using --clean. False positives are possible.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if !clean {
			fmt.Printf("Run 'binds detect-pollution --clean' to delete these issues (with confirmation).\n")
