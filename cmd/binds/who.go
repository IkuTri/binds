...
func serverClient() (*http.Client, string, error) {
	...
	if env := os.Getenv("BINDS_SERVER_URL"); env != "" {
		return &http.Client{Timeout: 10 * time.Second}, strings.TrimRight(env, "/"), nil
	}

	home, _ := os.UserHomeDir()
	if home != "" {
		if cfg, err := server.LoadConfigFile(filepath.Join(home, ".config", "binds")); err == nil {
			if cfg.Server.Host != "" {
