...
Custom Status States:
  ...
  Example:
    binds config set status.custom "awaiting_review,awaiting_testing,awaiting_docs"
...
Examples:
  binds config set jira.url "https://company.atlassian.net"
  binds config set jira.project "PROJ"
  binds config set status.custom "awaiting_review,awaiting_testing"
  binds config get jira.url
  binds config list
  binds config unset jira.url`,
}
...
// This addresses the confusion when `binds config list` shows one value but the effective
...
Examples:
  binds config validate
  binds config validate --json`,
	Run: func(cmd *cobra.Command, args []string) {
		cwd, err := os.Getwd()
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		...
		fmt.Println("\nRun 'binds config set <key> <value>' to fix configuration issues.")
		os.Exit(1)
	},
}
