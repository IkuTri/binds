	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for scanner.Scan() {
			...
			if bytes.HasPrefix(trimmed, []byte("<<<<<<< ")) ||
				...
				// Attempt automatic merge using binds merge command
		...
		if err != nil {
			...
			if result != nil && result.PrefixMismatch {
				...
				fmt.Fprintf(os.Stderr, "\nOr use 'binds rename-prefix' after import to fix the database.\n")
				os.Exit(1)
			}
		...
		if dedupeAfter {
			...
			for i, group := range duplicateGroups {
				...
				fmt.Fprintf(os.Stderr, "  Suggested: binds merge %s --into %s\n\n",
					strings.Join(sources, " "), target.ID)
			}

			fmt.Fprintf(os.Stderr, "Run 'binds duplicates --auto-merge' to merge all duplicates.\n")
		}
	},
}
...
// preventing binds doctor and validatePreExport from incorrectly warning that JSONL is newer.
//
...
// attemptAutoMerge attempts to resolve git conflicts using binds merge 3-way merge.
...
func attemptAutoMerge(conflictedPath string) error {
	...
	// Get current executable to call binds merge
	exe, err := os.Executable()
	if err != nil {
		return fmt.Errorf("cannot resolve current executable: %w", err)
	}

	// Invoke binds merge command
	...
	if err != nil {
		// Check exit code - binds merge returns 1 if there are conflicts, 2 for errors
		if exitErr, ok := err.(*exec.ExitError); ok {
