...
Examples:
  ...
  bd info --thanks`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("\nBinds Database Information")
		fmt.Println("===========================")
		fmt.Printf("Database: %s\n", absDBPath)
		fmt.Printf("Mode: %s\n", daemonStatus.Mode)

		fmt.Println("\nDaemon Status:")
		fmt.Printf("  Connected: no (daemon removed)\n")
...
var versionChanges = []VersionChange{
	{
		...
		Date:    "2026-04-20",
		Changes: []string{
			"FORK: binds forked from beads v0.49.4 (steveyegge/beads@3032c622)",
			"CHANGE: Renamed binary from bd to binds",
			"CHANGE: Repositioned as agent coordination & work tracking layer",
			"PHASE-1: Agent coordination primitives (rooms, presence, mail) in development",
		},
	},
	{
		Version: "0.49.3",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds doctor --server - Dolt server mode health checks",
			...
			"FIX: binds list --json optimization - Fetch only needed dependencies (#1316)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds config validate - Validate sync configuration",
			"NEW: Jujutsu (jj) VCS support - Binds now works with jj repositories",
			"NEW: Per-field merge strategies for conflict resolution",
			"NEW: -m flag as alias for --description in binds create",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds close -m flag - Alias for --reason (git commit convention)",
	...
	},
	{
		...
		Date:    "2026-01-12",
		Changes: []string{
			"NEW: binds list --ready flag - Show only issues with no blockers (bd-ihu31)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds create --dry-run - Preview issue creation without side effects (bd-0hi7)",
			...
			"NEW: Multi-repo custom types - binds doctor discovers types across repos (bd-62g22)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: Deep pre-commit integration - binds doctor checks pre-commit configs (bd-28r5)",
			"NEW: Rig identity bead type - New 'rig' type for Gas Town tracking (gt-zmznh)",
			"NEW: --filter-parent alias - Alternative to --parent in binds list (bd-3p4u)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: --blocks flag for binds dep add - Natural dependency syntax (GH#884)",
			"NEW: --blocked-by/--depends-on aliases for binds dep add (bd-09kt)",
	...
	},
	{
		...
		Changes: []string{
			...
			"FIX: binds list defaults - Non-closed issues, 50 limit (GH#788)",
	...
	},
	{
		...
		Changes: []string{
			...
			"FIX: binds doctor pre-push hook detection (GH#799)",
	...
	},
	{
		...
		Changes: []string{
			...
			"FIX: binds doctor follows redirects - Multi-clone compatibility",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: --prefix flag for binds create - Create issues in other rigs from any directory",
			...
			"CHANGED: binds doctor --check=pollution - Test pollution check integrated into doctor",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: Cross-rig ID auto-resolve - binds dep add auto-resolves IDs across rigs",
			...
			"NEW: Database corruption recovery in binds doctor --fix - Auto-repair corrupted databases",
			"NEW: JSONL integrity check in binds doctor - Detect and fix malformed JSONL",
			"NEW: Git hygiene checks in binds doctor - Detect stale branches and sync issues",
			...
			"FIX: --resolution alias restored for binds close - Backwards compatibility",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds close --suggest-next - Show newly unblocked issues after close",
			...
			"NEW: Stale molecules check in binds doctor - Proactive detection",
			"NEW: Distinct ID prefixes - bd-proto-xxx, bd-mol-xxx, bd-wisp-xxx",
			"NEW: no-git-ops config - binds config set no-git-ops true for manual git control",
			...
			"FIX: installed_plugins.json v2 format - binds doctor handles new Claude Code format",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds list --pretty --watch - Built-in colorized viewer with live updates",
			...
			"NEW: --resolution alias for --reason on binds close",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: binds list --parent flag - Filter issues by parent",
			...
			"IMPROVED: binds doctor --fix replaces manual commands",
			"IMPROVED: binds dep tree shows external dependencies",
			...
			"FIX: binds create -f works with daemon mode",
	...
	},
	{
		...
		Date:    "2025-12-21",
		Changes: []string{
			"NEW: Wisp molecules - use 'binds wisp create' for ephemeral wisps",
			"NEW: Wisp issues live only in SQLite, never export to JSONL (prevents zombie resurrection)",
			"NEW: Use 'binds pour' for persistent mols, 'binds wisp create' for ephemeral wisps",
	...
	},
	{
		...
		Changes: []string{
			...
			"CHANGED: Thin hook shims - Hooks delegate to binds hooks run, no more version drift",
	...
	},
	{
		...
		Date:    "2025-12-18",
		Changes: []string{
			"binds graph command shows dependency counts using subgraph formatting",
	...
	},
	{
		...
		Date:    "2025-12-18",
		Changes: []string{
			"REMOVED: YAML simple template system - --from-template flag removed from binds create",
			"REMOVED: Embedded templates (bug.yaml, epic.yaml, feature.yaml) - Use Binds templates instead",
			"Templates are now purely Binds-based - Create epic with 'template' label, use bd template instantiate",
		},
	},
	{
		Version: "0.30.4",
		Date:    "2025-12-18",
		Changes: []string{
			"binds template instantiate - Create binds issues from Binds templates",
			"--assignee flag for template instantiate - Auto-assign during instantiation",
			"binds mail inbox --identity fix - Now properly filters by identity parameter",
	...
	},
	{
		...
		Date:    "2025-12-16",
		Changes: []string{
			"binds setup droid - Factory.ai (Droid) IDE support",
	...
	},
	{
		...
		Date:    "2025-12-16",
		Changes: []string{
			"binds reset command - Complete beads removal from a repository",
			"binds update --type flag - Change issue type after creation",
			"binds q silent mode - Quick-capture without output for scripting",
			"binds show displays dependent issue status - Shows status for blocked-by/blocking issues",
			...
			"binds migrate-tombstones command - Converts legacy deletions.jsonl to inline tombstones",
	...
	},
	{
		...
		Changes: []string{
			...
			"binds migrate-tombstones - Convert legacy deletions.jsonl to inline tombstones",
			"binds doctor tombstone health checks - Detects orphaned/expired tombstones",
			...
			"binds thanks command - List contributors to your project",
	...
	},
	{
		...
		Date:    "2025-12-03",
		Changes: []string{
			"--estimate flag for binds create/update - Add time estimates to issues in minutes",
			"binds doctor improvements - SQLite integrity check, config validation, stale sync branch detection",
			"binds doctor --output flag - Export diagnostics to file for sharing/debugging",
			"binds doctor --dry-run flag - Preview fixes without applying them",
			"binds doctor per-fix confirmation mode - Approve each fix individually",
			"--readonly flag - Read-only mode for worker sandboxes",
			"binds sync safety improvements - Auto-push after merge, diverged history handling",
	...
	},
	{
		...
		Date:    "2025-12-01",
		Changes: []string{
			"binds daemon --local flag - Run daemon without git operations for multi-repo/worktree setups",
			"binds daemon --foreground flag - Run in foreground for systemd/supervisord integration",
			"binds migrate-sync command - Migrate to sync.branch workflow for cleaner main branch",
	...
	},
	{
		...
		Changes: []string{
			...
			"binds sync --squash - Batch multiple sync commits into one",
			"Fresh Clone Detection - binds doctor suggests 'binds init' when JSONL exists but no DB",
		},
	},
	{
		Version: "0.26.0",
		Date:    "2025-11-27",
		Changes: []string{
			"binds doctor --check-health - Lightweight health checks for startup hooks (exit 0 on success)",
			...
			"binds sync Commit Scope Fixed - Only commits .beads/ files, not other staged files",
		},
	},
	{
		Version: "0.25.1",
		...
		Changes: []string{
			...
			"binds sync commit scope fixed - Now commits entire .beads/ directory before pull",
			"binds prime ephemeral branch detection - Auto-detects ephemeral branches and adjusts workflow",
	...
	},
	{
		...
		Changes: []string{
			...
			"Sorting for binds list/search - New `--sort` and `--reverse` flags",
			"Claude Integration Verification - New binds doctor checks",
	...
	},
	{
		...
		Date:    "2025-11-22",
		Changes: []string{
			"binds search filters - Date and priority filters added",
			"binds count - New command for counting and grouping issues",
	...
	},
	{
		...
		Date:    "2025-11-20",
		Changes: []string{
			"binds doctor --fix - Automatic repair functionality",
			"binds clean - Remove temporary merge artifacts",
	...
	},
	{
		...
		Changes: []string{
			...
			"`binds hooks install` - Embedded git hooks command (replaces external script)",
			"`bd cleanup` - Bulk deletion for agent-driven compaction",
			"`bd new` alias added - Agents often tried this instead of `binds create`",
			"`binds list` now one-line-per-issue by default - Prevents agent miscounting (use --long for old format)",
