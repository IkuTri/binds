...
// hookCmd is the main "binds hook" command that git hooks call into.
// This is distinct from "binds hooks" (plural) which manages hook installation.
...
func hookPostMergeDolt(beadsDir string) int {
	...
	if len(conflicts) > 0 {
		fmt.Fprintf(os.Stderr, "Warning: %d conflict(s) detected during Dolt merge; resolve with 'binds federation conflicts' or Dolt conflict tooling\n", len(conflicts))
