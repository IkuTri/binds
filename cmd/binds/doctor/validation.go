...
func CheckMergeArtifacts(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds doctor --fix' to remove merge artifacts",
	}
}
...
func CheckOrphanedDependencies(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Detail:  detail,
		Fix:     "Run 'binds doctor --fix' to remove orphaned dependencies",
	}
}
...
func CheckDuplicateIssues(path string, gastownMode bool, gastownThreshold int) DoctorCheck {
	...
	if totalDuplicates > threshold {
		return DoctorCheck{
			...
			Fix:     "Run 'binds duplicates' to review and merge duplicates",
		}
	}
...
func CheckTestPollution(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds doctor --check=pollution' to review and clean test issues",
	}
}
...
func CheckChildParentDependencies(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Detail:   detail,
		Fix:      "Run 'binds doctor --fix --fix-child-parent' to remove (if unintentional)",
		Category: CategoryMetadata,
	}
}
