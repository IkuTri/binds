...
type PendingMigration struct {
	...
	Command     string // e.g., "binds migrate hash-ids"
...
func DetectPendingMigrations(path string) []PendingMigration {
	...
	if needsHashIDsMigration(beadsDir) {
		pending = append(pending, PendingMigration{
			...
			Command:     "binds migrate hash-ids",
			Priority:    2,
		})
	}
	...
	if needsTombstonesMigration(beadsDir) {
		pending = append(pending, PendingMigration{
			...
			Command:     "binds migrate tombstones",
			Priority:    2,
		})
	}
	...
	if needsSyncMigration(path) {
		pending = append(pending, PendingMigration{
			...
			Command:     "binds migrate sync beads-sync",
			Priority:    3,
		})
	}
