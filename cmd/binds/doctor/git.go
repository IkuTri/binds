...
// These hooks have the logic embedded directly rather than calling binds hooks run
const bdInlineHookMarker = "# bd (beads)"

// bdHooksRunPattern matches hooks that call binds hooks run
...
func CheckGitHooks() DoctorCheck {
	...
	if len(externalManagers) > 0 {
		...
		if hasBdShims, bdHooks := areBdShimsInstalled(hooksDir); hasBdShims {
			return DoctorCheck{
				Name:    "Git Hooks",
				Status:  StatusOK,
				Message: "binds shims installed (ignoring external manager config)",
				Detail:  fmt.Sprintf("binds hooks run: %s", strings.Join(bdHooks, ", ")),
			}
		}
			...
			if integration.Configured {
				...
				if len(integration.HooksWithoutBd) > 0 {
					return DoctorCheck{
						...
						Fix:     "Add or upgrade to 'binds hooks run <hook>'. See " + hooksUpgradeURL,
					}
				}
				...
				return DoctorCheck{
					...
					Detail:  fmt.Sprintf("binds hooks run: %s", strings.Join(integration.HooksWithBd, ", ")),
				}
			}
	...
	hookInstallMsg := "Install hooks with 'binds hooks install'. See " + hooksExamplesURL
...
// areBdShimsInstalled checks if the installed hooks are bd shims, call binds hooks run,
// or are inline binds hooks created by bd init.
// This helps detect when binds hooks are installed directly but an external manager config exists.
// Returns (true, installedHooks) if binds hooks are detected, (false, nil) otherwise.
// (GH#946, GH#1120)
func areBdShimsInstalled(hooksDir string) (bool, []string) {
	...
	for _, hookName := range hooks {
		...
		// Check for bd-shim marker, binds hooks run call, or inline bd hook marker (from bd init)
...
func CheckGitUpstream(path string) DoctorCheck {
	...
	if behind > 0 && ahead == 0 {
		return DoctorCheck{
			...
			Detail:  fmt.Sprintf("Branch: %s, upstream: %s", branch, upstream),
			Fix:     "Run 'git pull --rebase' (then re-run bd sync / binds doctor)",
		}
	}
...
// (added in version 0.29.0). Without it, users experience circular "binds sync" failures (issue #532).
func CheckSyncBranchHookCompatibility(path string) DoctorCheck {
	...
	if !strings.Contains(hookStr, "bd-hooks-version:") {
		...
		return DoctorCheck{
			...
			Fix: "Either run 'binds hooks install --force' to use binds hooks,\n" +
	...
	if hookVersion == "" {
		return DoctorCheck{
			...
			Fix:     "Run 'binds hooks install --force' to update hooks",
		}
	}

	// MinSyncBranchHookVersion added sync-branch bypass logic
	// If hook version < MinSyncBranchHookVersion, it will cause circular "binds sync" failures
	if CompareVersions(hookVersion, MinSyncBranchHookVersion) < 0 {
		return DoctorCheck{
			...
			Detail:  fmt.Sprintf("Hook version %s lacks sync-branch bypass (requires %s+). This causes circular 'binds sync' failures during push.", hookVersion, MinSyncBranchHookVersion),
			Fix:     "Run 'binds hooks install --force' to update hooks",
		}
	}
...
func CheckMergeDriver(path string) DoctorCheck {
	...
	if err != nil {
		...
		return DoctorCheck{
			...
			Fix:     "Run 'binds init' to configure the merge driver, or manually: git config merge.beads.driver \"binds merge %A %O %A %B\"",
		}
	}

	currentConfig := strings.TrimSpace(string(output))
	correctConfig := "binds merge %A %O %A %B"
	...
	if strings.Contains(currentConfig, "%L") || strings.Contains(currentConfig, "%R") {
		return DoctorCheck{
			...
			Fix:     "Run 'binds doctor --fix' to update to correct config, or manually: git config merge.beads.driver \"binds merge %A %O %A %B\"",
		}
	}
...
func CheckSyncBranchConfig(path string) DoctorCheck {
	...
	if hasRemote {
		return DoctorCheck{
			...
			Fix:     "Run 'binds migrate sync beads-sync' to set up sync branch workflow",
		}
	}
...
func CheckSyncBranchHealth(path string) DoctorCheck {
	...
	if err != nil {
		...
		return DoctorCheck{
			...
			Fix:     "Run 'binds doctor --fix' to reset sync branch",
		}
	}
...
func CheckGitHooksDoltCompatibility(path string) DoctorCheck {
	...
	// Shim hooks (bd-shim) delegate to 'binds hook' which handles Dolt correctly
	if strings.Contains(contentStr, bdShimMarker) {
		return DoctorCheck{
			Name:    "Git Hooks Dolt Compatibility",
			Status:  StatusOK,
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds hooks install --force' to update hooks for Dolt compatibility",
	}
}

// FixGitHooks fixes missing or broken git hooks by calling binds hooks install.
...
// This is the shared core logic used by both 'binds orphans' and 'binds doctor' commands.
...
// message but forgot to run "binds close".
func CheckOrphanedIssues(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'binds show <id>' to check if implemented, then 'binds close <id>' if done",
		Category: CategoryGit,
	}
}
