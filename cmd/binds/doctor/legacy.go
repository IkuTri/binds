...
func CheckLegacyBeadsSlashCommands(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix: "Migrate to bd prime hooks for better token efficiency:\n" +
			"\n" +
			"Migration Steps:\n" +
			"  1. Run 'binds setup claude' to add SessionStart/PreCompact hooks\n" +
			"  2. Update AGENTS.md/CLAUDE.md:\n" +
			"     - Remove /beads:* slash command references\n" +
			"     - Add: \"Run 'binds prime' for workflow context\" (for users without hooks)\n" +
			"\n" +
			"Benefits:\n" +
...
func CheckAgentDocumentation(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix: "Add agent documentation:\n" +
			"  • Run 'binds onboard' to create AGENTS.md with workflow guidance\n" +
			"  • Or run 'binds setup claude' to add Claude-specific documentation\n" +
			"\n" +
	...
	if cfg.JSONLExport == "beads.jsonl" {
		...
		if legacyExists && !canonicalExists {
			return DoctorCheck{
				...
				Fix: "Run 'binds doctor --fix' to auto-migrate, or manually:\n" +
		...
		if !legacyExists && canonicalExists {
			...
			return DoctorCheck{
				...
				Fix:     "Run 'binds doctor --fix' to update the configuration",
			}
		}
	}
...
func CheckDatabaseConfig(repoPath string) DoctorCheck {
	...
	if cfg.JSONLExport != "" {
		if cfg.JSONLExport == "deletions.jsonl" || cfg.JSONLExport == "interactions.jsonl" || cfg.JSONLExport == "molecules.jsonl" {
			return DoctorCheck{
				...
				Fix:     "Run 'binds doctor --fix' to reset metadata.json jsonl_export to issues.jsonl, then commit the change.",
			}
		}
	...
	return DoctorCheck{
		...
		Fix: "Run 'binds doctor --fix' to auto-detect and fix mismatches, or manually:\n" +
...
// CheckFreshClone detects if this is a fresh clone that needs 'binds init'.
...
func CheckFreshClone(repoPath string) DoctorCheck {
	...
	fixCmd := "binds init"
	if prefix != "" {
		fixCmd = fmt.Sprintf("binds init --prefix %s", prefix)
	}
	if backend == configfile.BackendDolt {
		fixCmd = "binds init --backend dolt"
		if prefix != "" {
			fixCmd = fmt.Sprintf("binds init --backend dolt --prefix %s", prefix)
		}
	}
	...
	return DoctorCheck{
		...
		Detail: "This appears to be a freshly cloned repository.\n" +
			...
			"  Run 'binds init' to create the database and import existing issues.",
