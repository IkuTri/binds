...
func CheckConfigValues(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Edit config files to fix invalid values. Run 'binds config' to view current settings.",
	}
}
...
func checkYAMLConfigValues(repoPath string) []string {
	...
	if v.IsSet("routing.mode") {
		...
		// so routed issues are visible in binds list via multi-repo hydration
		if mode == "auto" {
				...
				if !hasHydration {
					issues = append(issues,
						"routing.mode=auto with routing targets but repos.additional not configured. "+
							"Issues created via routing will not be visible in binds list. "+
							"Run 'binds repo add <routing-target>' to enable hydration.")
				} else {
						...
						if !additionalSet[expandedContributor] {
							issues = append(issues, fmt.Sprintf(
								"routing.contributor=%q is not in repos.additional. "+
									"Run 'binds repo add %s' to make routed issues visible.",
