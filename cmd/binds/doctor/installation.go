...
func CheckInstallation(path string) DoctorCheck {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		...
		return DoctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'binds init --prefix %s' to initialize beads", prefix),
		}
	}
...
func CheckMultipleDatabases(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds migrate' to consolidate databases or manually remove old .db files",
	}
}
...
func CheckPermissions(path string) DoctorCheck {
	...
	if err := os.WriteFile(testFile, []byte("test"), 0600); err != nil {
		return DoctorCheck{
			...
			Fix:     "Run 'binds doctor --fix' to fix permissions",
		}
	}
...
func CheckUntrackedBeadsFiles(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds doctor --fix' to stage and commit untracked files, or manually: git add .beads/*.jsonl && git commit",
	}
}
