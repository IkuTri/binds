...
func hasBeadsHooks(settingsPath string) bool {
	...
	// Check SessionStart and PreCompact for "binds prime"
...
func VerifyPrimeOutput() DoctorCheck {
	...
	} else if !hasMCP && strings.Contains(outputStr, "binds ready") {
		return DoctorCheck{
			Name:    "binds prime Output",
			Status:  "ok",
...
// This is important because Claude hooks rely on executing 'binds prime'.
...
// CheckDocumentationBdPrimeReference checks if AGENTS.md or CLAUDE.md reference 'binds prime'
...
func CheckDocumentationBdPrimeReference(repoPath string) DoctorCheck {
	...
	for _, docFile := range docFiles {
		...
		if strings.Contains(string(content), "binds prime") {
	...
	if err := cmd.Run(); err != nil {
		return DoctorCheck{
			Name:    "Prime Documentation",
			Status:  "warning",
			Message: "Documentation references 'binds prime' but command not found",
