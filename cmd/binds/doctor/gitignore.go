...
func CheckGitignore() DoctorCheck {
	...
	if err != nil {
		return DoctorCheck{
			...
			Fix:     "Run: bd init (safe to re-run) or binds doctor --fix",
		}
	}
	...
	if len(missing) > 0 {
		return DoctorCheck{
			...
			Fix:     "Run: binds doctor --fix or bd init (safe to re-run)",
		}
	}
...
func CheckRedirectNotTracked() DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds doctor --fix' to untrack, or manually: git rm --cached .beads/redirect",
	}
}
	...
	if _, err := os.Stat(dbPath); os.IsNotExist(err) {
		...
		if len(matches) == 0 {
			return DoctorCheck{
				...
				Detail:  fmt.Sprintf("Target: %s", resolvedTarget),
				Fix:     "Run 'binds init' in the target directory or check redirect path",
			}
		}
	}
...
func CheckRedirectTargetSyncWorktree() DoctorCheck {
	...
	if _, err := os.Stat(worktreePath); os.IsNotExist(err) {
		return DoctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'binds sync' in %s to create the worktree", targetRepoRoot),
		}
	}
...
func CheckLastTouchedNotTracked() DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds doctor --fix' to untrack, or manually: git rm --cached .beads/last-touched",
	}
}
...
func CheckSyncBranchGitignore() DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'binds doctor --fix' or 'binds sync' to set git index flags",
	}
}
