...
// This enables agents to use 'binds mail' consistently, while the actual
...
Agents often type 'binds mail' when working with beads, but mail functionality
...
Configuration (checked in order):
  1. BINDS_MAIL_DELEGATE or BD_MAIL_DELEGATE environment variable
  2. 'mail.delegate' config setting (binds config set mail.delegate "gt mail")

Examples:
  # Configure delegation (one-time setup)
  export BINDS_MAIL_DELEGATE="gt mail"
  # or
  binds config set mail.delegate "gt mail"
  ...
  bd mail read msg-123             # Reads a message`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if delegate == "" {
			...
			fmt.Fprintf(os.Stderr, "binds mail delegates to an external mail provider.\n")
			fmt.Fprintf(os.Stderr, "Configure one of:\n")
			fmt.Fprintf(os.Stderr, "  export BINDS_MAIL_DELEGATE=\"gt mail\"   # Environment variable\n")
			fmt.Fprintf(os.Stderr, "  binds config set mail.delegate \"gt mail\"  # Per-project config\n")
			os.Exit(1)
		}
...
// Priority: env vars > binds config
func findMailDelegate() string {
	// Check environment variables first
	if delegate := os.Getenv("BINDS_MAIL_DELEGATE"); delegate != "" {
		return delegate
	}
	...
	// Check binds config (requires database)
