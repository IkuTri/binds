...
var doctorCmd = &cobra.Command{
	Use:     "doctor [path]",
	GroupID: "maint",
	Short:   "Check and fix binds installation health (start here)",
...
Performance Mode (--perf):
  ...
  - Times key operations (bd ready, binds list, bd show, etc.)
...
Deep Validation Mode (--deep):
  ...
  - Agent bind integrity: Agent binds have valid state values
...
Server Mode (--server):
  ...
  - Database exists: Does the 'binds' database exist?
  - Schema compatible: Can query binds tables?
...
Examples:
  binds doctor              # Check current directory
  binds doctor /path/to/repo # Check specific repository
  binds doctor --json       # Machine-readable output
  binds doctor --fix        # Automatically fix issues (with confirmation)
  binds doctor --fix --yes  # Automatically fix issues (no confirmation)
  binds doctor --fix -i     # Confirm each fix individually
  binds doctor --fix --fix-child-parent  # Also fix child→parent deps (opt-in)
  binds doctor --fix --force # Force repair even when database can't be opened
  binds doctor --fix --source=jsonl # Rebuild database from JSONL (source of truth)
  binds doctor --dry-run    # Preview what --fix would do without making changes
  binds doctor --perf       # Performance diagnostics
  binds doctor --output diagnostics.json  # Export diagnostics to file
  binds doctor --check=pollution          # Show potential test issues
  binds doctor --check=pollution --clean  # Delete test issues (with confirmation)
  binds doctor --deep             # Full graph integrity validation
  binds doctor --server           # Dolt server mode health checks
  binds doctor --migration=pre    # Validate readiness for Dolt migration
  binds doctor --migration=post   # Validate Dolt migration completed
  binds doctor --migration=pre --json  # Machine-parseable migration validation`,
