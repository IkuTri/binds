  ...
  bd upgrade status --json`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if versionUpgradeDetected {
			...
			fmt.Printf("binds version: v%s (first run or version tracking just enabled)\n", Version)
		} else {
			fmt.Printf("binds version: v%s (no upgrade detected)\n", Version)
		}
	},
}
  ...
  bd upgrade review --json`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if lastVersion == "" {
			...
			fmt.Println("Run 'binds info --whats-new' to see recent changes")
			return
		}
		...
		if len(newVersions) == 0 {
			...
			fmt.Println("Run 'binds info --whats-new' to see recent documented changes")
			return
		}
		...
		fmt.Println("💡 Run 'binds upgrade ack' to mark this version as seen")
		fmt.Println()
	},
}
