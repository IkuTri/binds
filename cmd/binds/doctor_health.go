...
func printCheckHealthHint(issues []string) {
	fmt.Fprintf(os.Stderr, "💡 binds doctor recommends a health check:\n")
	for _, issue := range issues {
		fmt.Fprintf(os.Stderr, "   • %s\n", issue)
	}
	fmt.Fprintf(os.Stderr, "   Run 'binds doctor' for details, or 'binds doctor --fix' to auto-repair\n")
	fmt.Fprintf(os.Stderr, "   (Suppress with: binds config set %s false)\n", ConfigKeyHintsDoctor)
	os.Exit(1)
}
