...
The backend is set at initialization time with 'binds init --backend <type>'.
To change backends, use 'binds migrate dolt' or reinitialize.
		...
		if jsonOutput {
			...
			if backend == configfile.BackendSQLite {
				...
				if isPreferDoltConfigured(beadsDir) {
					...
					result["migration_command"] = "binds migrate dolt"
				}
			}
