...
func warnIfCyclesExist(s storage.Storage) {
	...
	fmt.Fprintf(os.Stderr, "\nRun 'binds dep cycles' for detailed analysis.\n\n")
}
...
When called with an issue ID and --blocks flag, creates a blocking dependency:
  binds dep <blocker-id> --blocks <blocked-id>

This is equivalent to:
  binds dep add <blocked-id> <blocker-id>

Examples:
  binds dep bd-xyz --blocks bd-abc    # bd-xyz blocks bd-abc
  binds dep add bd-abc bd-xyz         # Same as above (bd-abc depends on bd-xyz)`,
...
The depends-on-id can be provided as:
  - A positional argument: binds dep add issue-123 issue-456
  - A flag: binds dep add issue-123 --blocked-by issue-456
  - A flag: binds dep add issue-123 --depends-on issue-456
...
Examples:
  binds dep add bd-42 bd-41                              # Positional args
  binds dep add bd-42 --blocked-by bd-41                 # Flag syntax (same effect)
  binds dep add bd-42 --depends-on bd-41                 # Alias (same effect)
  binds dep add gt-xyz external:beads:mol-run-assignee   # Cross-project dependency`,
...
Examples:
  binds dep list gt-abc                     # Show what gt-abc depends on
  binds dep list gt-abc --direction=up      # Show what depends on gt-abc
  binds dep list gt-abc --direction=up -t tracks  # Show what tracks gt-abc (convoy tracking)`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if direction == "up" {
			issues, issuesErr = store.GetDependentsWithMetadata(ctx, fullID)
		} else {
			issues, issuesErr = store.GetDependenciesWithMetadata(ctx, fullID)
		}
		if issuesErr != nil {
			FatalErrorRespectJSON("%v", issuesErr)
		}
...
Examples:
  binds dep tree gt-0iqq                    # Show what blocks gt-0iqq
  binds dep tree gt-0iqq --direction=up     # Show what gt-0iqq blocks
  binds dep tree gt-0iqq --status=open      # Only show open issues
  binds dep tree gt-0iqq --depth=3          # Limit to 3 levels deep`,
	Args: cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
		ctx := rootCtx
		...
		if direction == "both" {
			...
			downTree, treeErr := store.GetDependencyTree(ctx, fullID, maxDepth, showAllPaths, false)
			if treeErr != nil {
				FatalErrorRespectJSON("%v", treeErr)
			}

			// Get dependents (up) - what this issue blocks
			upTree, treeErr2 := store.GetDependencyTree(ctx, fullID, maxDepth, showAllPaths, true)
			if treeErr2 != nil {
				FatalErrorRespectJSON("%v", treeErr2)
			}
...
func init() {
	...
	depCmd.Flags().StringP("blocks", "b", "", "Issue ID that this issue blocks (shorthand for: binds dep add <blocked> <blocker>)")
