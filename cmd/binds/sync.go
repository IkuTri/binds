	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if squash {
			if dryRun {
				...
				fmt.Println("  Run 'binds sync' (without --squash) to commit all accumulated changes")
			}
			return
		}
		...
		if inMerge, err := gitHasUnmergedPaths(); err != nil {
			...
			FatalErrorWithHint("unmerged paths or merge in progress", "resolve conflicts, run 'binds import' if needed, then 'binds sync' again")
		}
...
func doExportSync(ctx context.Context, jsonlPath string, force, dryRun bool) error {
	...
	if shouldUseDolt {
		...
		if !ok {
			...
			if err := rs.Commit(ctx, "binds sync: auto-commit"); err != nil {
...
func init() {
	...
	syncCmd.Flags().Bool("squash", false, "Accumulate changes in JSONL without committing (run 'binds sync' later to commit all)")
