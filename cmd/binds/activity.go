...
// without the daemon (which has been removed).
const daemonUnavailableForActivity = "Error: single-rig activity requires a daemon which has been removed.\nUse --town for aggregated feed from all rigs (uses routes.jsonl)"
...
func runActivity(cmd *cobra.Command, args []string) {
	...
	// Single-rig activity requires daemon (daemon removed — always errors)
	fmt.Fprintln(os.Stderr, daemonUnavailableForActivity)
	os.Exit(1)
}
...
func runTownActivityOnce(sinceTime time.Time) {
	...
	if activeCount == 0 {
		...
		fmt.Fprintln(os.Stderr, "Hint: Start daemons with 'binds daemons start' in each rig")
		os.Exit(1)
	}
