...
var (
	...
	// Direct mode only — daemon removed
...
func init() {
	...
	rootCmd.PersistentFlags().BoolVar(&noDaemon, "no-daemon", false, "No-op: daemon removed; kept for flag compatibility")
	...
	// should eventually be consolidated into 'binds doctor' and 'binds doctor --fix' to simplify
...
var rootCmd = &cobra.Command{
	Use:   "binds",
	Short: "binds - Agent coordination & work tracking",
	Long:  `binds — modular, agent-agnostic coordination layer for AI coding agents. Local-first. Single binary.`,
	Run: func(cmd *cobra.Command, args []string) {
		// Handle --version flag on root command
		if v, _ := cmd.Flags().GetBool("version"); v {
			fmt.Printf("binds version %s (%s)\n", Version, Build)
			return
		}
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		// like "binds version" that don't need database access.
		noDbCommands := []string{
			cmdDaemon,
		...
		// Initialize daemon status (always direct mode — daemon removed)
		daemonStatus = DaemonStatus{
			Mode:           "direct",
			Connected:      false,
			Degraded:       false,
			FallbackReason: FallbackFlagNoDaemon,
		}
		...
		if backend == configfile.BackendDolt {
			...
			if err != nil && useReadOnly {
				...
				// This handles the case where user runs "binds list" before "binds init"
