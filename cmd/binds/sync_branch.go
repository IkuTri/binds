...
func getSyncBranch(ctx context.Context) (string, error) {
	...
	if syncBranch == "" {
		return "", fmt.Errorf("sync.branch not configured (run 'binds config set sync.branch <branch-name>')")
	}
...
func mergeSyncBranch(ctx context.Context, dryRun bool) error {
	...
	fmt.Println("2. Run 'binds sync --import-only' to sync the database with merged JSONL")
	fmt.Println("3. Run 'binds sync' to push changes to remote")
...
func commitToExternalBeadsRepo(ctx context.Context, beadsDir, message string, push bool) (bool, error) {
	...
	if message == "" {
		message = fmt.Sprintf("binds sync: %s", time.Now().Format("2006-01-02 15:04:05"))
	}
