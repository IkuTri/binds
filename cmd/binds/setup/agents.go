...
// AGENTS.md integration markers for binds section
const (
	agentsBeginMarker = "<!-- BEGIN BINDS INTEGRATION -->"
	agentsEndMarker   = "<!-- END BINDS INTEGRATION -->"
)

const agentsBeadsSection = `<!-- BEGIN BINDS INTEGRATION -->
## Issue Tracking with bd (binds)

**IMPORTANT**: This project uses **bd (binds)** for ALL issue tracking. Do NOT use markdown TODOs, task lists, or other tracking methods.
...
` + "```bash" + `
binds create "Issue title" --description="Detailed context" -t bug|feature|task -p 0-4 --json
binds create "Issue title" --description="What this issue is about" -p 1 --deps discovered-from:bd-123 --json
` + "```" + `
...
` + "```bash" + `
binds close bd-42 --reason "Completed" --json
` + "```" + `
...
4. **Discover new work?** Create linked issue:
   - ` + "`binds create \"Found bug\" --description=\"Details about what was found\" -p 1 --deps discovered-from:<parent-id>`" + `
5. **Complete**: ` + "`binds close <id> --reason \"Done\"`" + `
...
<!-- END BINDS INTEGRATION -->
`

var (
	errAgentsFileMissing   = errors.New("agents file not found")
	errBeadsSectionMissing = errors.New("binds section missing")
)
...
func installAgents(env agentsEnv, integration agentsIntegration) error {
	...
	if currentContent != "" {
		if strings.Contains(currentContent, agentsBeginMarker) {
			...
			_, _ = fmt.Fprintln(env.stdout, "✓ Updated existing binds section in AGENTS.md")
		} else {
			...
			_, _ = fmt.Fprintln(env.stdout, "✓ Added binds section to existing AGENTS.md")
		}
	} else {
		...
		_, _ = fmt.Fprintln(env.stdout, "✓ Created new AGENTS.md with binds integration")
	}
...
func checkAgents(env agentsEnv, integration agentsIntegration) error {
	...
	if strings.Contains(content, agentsBeginMarker) {
		...
		_, _ = fmt.Fprintln(env.stdout, "  Binds section found in AGENTS.md")
		return nil
	}

	_, _ = fmt.Fprintln(env.stdout, "⚠ AGENTS.md exists but no binds section found")
	_, _ = fmt.Fprintf(env.stdout, "  Run: %s (to add binds section)\n", integration.setupCommand)
...
func removeAgents(env agentsEnv, integration agentsIntegration) error {
	...
	content := string(data)
	if !strings.Contains(content, agentsBeginMarker) {
		_, _ = fmt.Fprintln(env.stdout, "No binds section found in AGENTS.md")
		return nil
	}
	...
	if trimmed == "" {
		...
		_, _ = fmt.Fprintf(env.stdout, "✓ Removed %s (file was empty after removing binds section)\n", env.agentsPath)
		return nil
	}
	...
	_, _ = fmt.Fprintln(env.stdout, "✓ Removed binds section from AGENTS.md")
	return nil
}

// updateBeadsSection replaces the binds section in existing content
...
// removeBeadsSection removes the binds section from content
