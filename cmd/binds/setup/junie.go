...
const junieGuidelinesTemplate = `# Binds Issue Tracking Instructions

This project uses **Binds (bd)** for issue tracking. Use the bd CLI or MCP tools for all task management.
...
binds list --status open # List all open issues
...
### Creating Issues
` + "```bash" + `
binds create "Title" --description="Details" -t bug|feature|task -p 0-4 --json
binds create "Found bug" --description="Details" --deps discovered-from:bd-42 --json
` + "```" + `
...
binds close <id> --reason "Completed"   # Mark complete
` + "```" + `

### Dependencies
` + "```bash" + `
binds dep add <issue> <depends-on>      # Add dependency (issue depends on depends-on)
binds dep add <issue> <depends-on> --type=related  # Soft link
` + "```" + `
...
func InstallJunie() {
	...
	fmt.Println("The MCP server provides direct access to binds tools.")
}
