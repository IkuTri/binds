...
const aiderConfigTemplate = `# Binds Issue Tracking Integration for Aider
# Auto-generated by 'binds setup aider'

# Load Binds workflow instructions for the AI
# This file is marked read-only and cached for efficiency
read:
  - .aider/BEADS.md
`

const aiderBeadsInstructions = `# Binds Issue Tracking Instructions for AI

This project uses **Binds (bd)** for issue tracking. Aider requires explicit command execution - suggest commands to the user.

## Core Workflow Rules

1. **Track ALL work in bd** (never use markdown TODOs or comment-based task lists)
2. **Suggest 'binds ready'** to find available work
3. **Suggest 'binds create'** for new issues/tasks/bugs
4. **Suggest 'binds sync'** at end of session
...
- ` + "`binds create --title=\"...\" --type=task`" + ` - Create new issue
...
3. **Do the work**
4. **Complete**: "Run ` + "`/run binds close <id>`" + ` when done"
...
2. Create new issues:
   ` + "```bash" + `
   /run binds create "Issue title" --description="Details" -t bug|feature|task -p 1
   ` + "```" + `
...
4. Complete work:
   ` + "```bash" + `
   /run binds close bd-42 --reason "Done"
   ` + "```" + `
