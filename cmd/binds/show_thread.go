...
func findRepliesTo(ctx context.Context, issueID string, s storage.Storage) string {
...
func findReplies(ctx context.Context, issueID string, s storage.Storage) []*types.Issue {
	if sqliteStore, ok := s.(*sqlite.SQLiteStorage); ok {
	...
	for childID, deps := range allDeps {
		for _, dep := range deps {
			if dep.Type == types.DepRepliesTo && dep.DependsOnID == issueID {
				issue, _ := s.GetIssue(ctx, childID)
				if issue != nil {
					replies = append(replies, issue)
				}
			}
