...
var agentBackfillLabelsCmd = &cobra.Command{
	Use:   "backfill-labels",
	Short: "Backfill role_type/rig labels on existing agent binds",
	Long: `Backfill role_type and rig labels on existing agent binds.

This command scans all agent binds and:
1. Extracts role_type and rig from description text if fields are empty
2. Sets the role_type and rig fields on the agent bind
3. Adds role_type:<value> and rig:<value> labels for filtering

This enables queries like:
  binds list --type=agent --label=role_type:witness
  binds list --type=agent --label=rig:gastown
...
func runAgentState(cmd *cobra.Command, args []string) error {
	...
	// Use routed resolution for cross-repo lookups (direct mode only — daemon removed)
	var err error
	...
	if err := activeStore.UpdateIssue(ctx, agentID, stateUpdates, actor); err != nil {
...
func runAgentHeartbeat(cmd *cobra.Command, args []string) error {
	...
	heartbeatUpdates := map[string]interface{}{
		"last_activity": time.Now(),
	}
	if err := activeStore.UpdateIssue(ctx, agentID, heartbeatUpdates, actor); err != nil {
...
func runAgentBackfillLabels(cmd *cobra.Command, args []string) error {
	...
	if len(agents) == 0 {
		fmt.Println("No agent binds found")
		return nil
	}
...
// emptyToNil returns nil if s is empty, otherwise returns s.
// Used for JSON output to emit null instead of empty string.
