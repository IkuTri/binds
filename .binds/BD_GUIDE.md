...
<!-- Run 'bd onboard --output .binds/BD_GUIDE.md' to regenerate -->
...
6. **Commit together**: Always commit the `.binds/issues.jsonl` file together with the code changes so issue state stays in sync with code state

### Auto-Sync

bd automatically syncs with git:
- Exports to `.binds/issues.jsonl` after changes (5s debounce)
...
├── docs/                # Documentation
└── .binds/
    ├── binds.db         # SQLite database (DO NOT COMMIT)
...
- ❌ Do NOT commit `.binds/binds.db` (JSONL only)
...
```bash
bd onboard --output .binds/BD_GUIDE.md
```
